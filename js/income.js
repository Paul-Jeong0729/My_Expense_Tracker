// income.js — 수입 "내역 보기" 전용 (입력은 구글 시트에서 직접 합니다)
// categories.js / sheets-api.js가 먼저 로드되어 있어야 해요.
...
function initCategoryTabs() {
  const wrap = document.getElementById('income-category-tabs');
  if (!wrap) return;
...
function getFilteredEntries() {
  const period = document.getElementById('income-period-select')?.value || 'current';
...
function renderFiltered() {
  ...
  const totalEl = document.getElementById('income-filtered-total');
...
async function refreshEntries() {
  try {
    allEntries = await SheetsAPI.list('Income');
    renderFiltered();
  } catch (err) {
    console.error(err);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const periodSelect = document.getElementById('income-period-select');
