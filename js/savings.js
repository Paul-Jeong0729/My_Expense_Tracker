// savings.js — 적금 페이지 전용 스크립트 (Google Sheets 기반)
// categories.js / sheets-api.js / ai-analysis.js / savings-advice.js가 먼저 로드되어야 해요.
// Expense 시트 중 적금·정기예탁·연금 항목만 모아서 트리 형태로 보여주고,
// 누적 금액과 목표 달성률을 계산합니다. (stats.html의 "저축 목표 달성률"과 같은
// Settings 값을 공유하므로 두 페이지의 %는 항상 같아요.)

const GOAL_SETTING_KEY = 'savings_goal';
const MANUAL_SETTING_KEY = 'savings_manual';
...
// categories.js의 EXPENSE_FIELD_MAP과 정확히 같은 category 문자열을 써야 매칭이 돼요.
const TREE_ITEMS = [
...
let lastSavingsDepositTotal = 0;
let lastPensionTotal = 0;
...
function renderTree() {
  ...
  lastSavingsDepositTotal = savingsDepositTotal;
  lastPensionTotal = pensionTotal;
...
function renderAdvice() {
  const container = document.getElementById('savings-ai-advice');
  if (!container) return;
  renderSavingsAdvice(container, {
    savingsDepositTotal: lastSavingsDepositTotal,
    pensionTotal: lastPensionTotal,
    goalTargetAmount,
    expenseEntries: latestExpense,
  });
}
...
function renderManualInputs() {
  MONTHLY_OVERRIDE_KEYS.forEach((key) => {
    ...
    if (input && document.activeElement !== input) input.value = savingsStartMonths[key] || '';
  });
...
function initStartMonthInputs() {
  document.querySelectorAll('.savings-start-input').forEach((input) => {
    input.addEventListener('change', async () => {
      ...
      try {
        const merged = await SheetsAPI.updateSettingMerge(MANUAL_SETTING_KEY, { startMonths: { [key]: value } });
        savingsStartMonths = merged.startMonths || {};
        renderTree();
      } catch (err) {
...
function initManualAmountInputs() {
  document.querySelectorAll('.savings-manual-input').forEach((input) => {
    input.addEventListener('change', async () => {
      ...
      try {
        const merged = await SheetsAPI.updateSettingMerge(MANUAL_SETTING_KEY, { manualAmounts: { [key]: value } });
        manualAmounts = merged.manualAmounts || {};
        renderTree();
...
async function refreshManualSetting() {
  try {
    const data = (await SheetsAPI.getSetting(MANUAL_SETTING_KEY)) || {};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.authReady.then((user) => {
    ...
    startPolling(refreshManualSetting, 30000);
  });
});
